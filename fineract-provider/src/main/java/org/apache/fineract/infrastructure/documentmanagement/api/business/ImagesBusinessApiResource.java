...
public class ImagesBusinessApiResource {
    ...
    public String retrieveAvatar(@PathParam("entity") final String entityName, @PathParam("entityId") final Long entityId) {
        ...
        } else if (ImagesApiResource.EntityTypeForImages.STAFF.toString().equalsIgnoreCase(entityName)) {
            this.context.authenticatedUser().validateHasReadPermission("STAFFIMAGE");
        }

        final CommandProcessingResult result = this.imageWritePlatformService.retrieveImage(entityName, entityId);
        return this.toApiJsonSerializer.serialize(result);
    }
