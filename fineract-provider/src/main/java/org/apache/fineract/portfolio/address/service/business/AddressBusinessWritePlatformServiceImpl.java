...
import org.apache.fineract.portfolio.address.domain.business.AddressOther;
import org.apache.fineract.portfolio.address.domain.business.AddressOtherRepositoryWrapper;
import org.apache.fineract.portfolio.address.exception.AddressNotFoundException;
import org.apache.fineract.portfolio.address.serialization.business.AddressBusinessCommandFromApiJsonDeserializer;
...
public class AddressBusinessWritePlatformServiceImpl implements AddressBusinessWritePlatformService {
    ...
    private final AddressBusinessCommandFromApiJsonDeserializer fromApiJsonDeserializer;
    ...
    public CommandProcessingResult addClientAddress(final Long clientId, final Long addressTypeId, final JsonCommand command) {
        ...
        return new CommandProcessingResultBuilder().withCommandId(command.commandId()).withSubEntityId(clientAddress.getId())
                .withEntityId(address.getId()).build();
    }
    ...
    public CommandProcessingResult updateClientAddress(final Long clientId, final JsonCommand command) {
        ...
        final Boolean testActive = command.booleanObjectValueOfParameterNamed("isActive");
        if (testActive != null) {
        ...
        return new CommandProcessingResultBuilder().withCommandId(command.commandId()).withSubEntityId(clientAddressObj.getId())
                .withEntityId(addressId).build();
    }
}
