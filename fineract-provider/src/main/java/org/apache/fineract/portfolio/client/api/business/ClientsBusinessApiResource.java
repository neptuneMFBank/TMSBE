...
public class ClientsBusinessApiResource {
    ...
    public String retrieveAll(final UriInfo uriInfo, final Long officeId, final String externalId, final String displayName,
        ...
        final SearchParametersBusiness searchParameters = SearchParametersBusiness.forClientsBusiness(officeId, externalId, statusId,
                ...
                email, mobile, legalFormId, bvn, nin);
    ...
    public String retrieveVisibileLoanProduct(@PathParam("clientId") final Long clientId) {
        ...
        final JsonObject LoanProducts = this.loanProductVisibilityReadPlatformService.retrieveVisibileProductForClient(clientId,
    ...
    public String retrieveClientTransactions(@Context final UriInfo uriInfo,
        ...
        LocalDate fromDate = DateUtils.getLocalDateOfTenant();
        if (startPeriod != null) {
            fromDate = startPeriod.getDate(LoanBusinessApiConstants.startPeriodParameterName, dateFormat, locale);
        }
        ...
        final SearchParametersBusiness searchParameters = SearchParametersBusiness.forTransactions(transactionTypeId, null, offset, limit, orderBy, sortOrder, fromDate, toDate, depositAccountTypeId);

        final JsonObject clientTransactions = this.clientBusinessReadPlatformService.retrieveClientTransactions(clientId, searchParameters);
        final Set<String> CLIENT_TRANSACTION_DATA_PARAMETERS = new HashSet<>(
                Arrays.asList("loanTransactions", "savingTransactions"
                ));
        final ApiRequestJsonSerializationSettings settings = this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
