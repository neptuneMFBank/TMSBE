...
@Service
public class LoanProductInterestRepositoryWrapper {

    private final LoanProductInterestRepository repository;

    @Autowired
    public LoanProductInterestRepositoryWrapper(final LoanProductInterestRepository repository) {
        this.repository = repository;
    }

    @Transactional(readOnly = true)
    public void saveAndFlush(final LoanProductInterest loanProductApproval) {
        this.repository.saveAndFlush(loanProductApproval);
    }

    @Transactional(readOnly = true)
    public LoanProductInterest findOneWithNotFoundDetection(final Long id) {
        return this.repository.findById(id).orElseThrow(() -> new LoanProductInterestNotFoundException(id));
    }
