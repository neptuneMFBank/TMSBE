...
public interface AppUserClientMappingRepository extends JpaRepository<AppUserClientMapping, Long> {
    ...
    List<AppUserClientMapping> findByClientId(long clientId);

    // Returns records that have matching values in both tables. Pictorial representation is given below:
    @Query(nativeQuery = true, value = "SELECT msucm.appuser_id FROM m_selfservice_user_client_mapping msucm INNER JOIN m_appuser ma ON msucm.appuser_id = ma.id WHERE msucm.client_id=client_id")
